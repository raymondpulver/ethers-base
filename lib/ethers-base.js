...
exports.EthersSuper = class EthersSuper extends Contract { // we don't explicitly extend Contract because we need to make JavaScript think we don't need to call super before getting "this", so we Object.setPrototypeOf below
  ...
  constructor(address, abi, providerOrSigner) {
    const { defineReadOnly } = properties;
    properties.defineReadOnly = (o, prop, value) => {
      if (typeof value !== 'function' || !o.functions) Object.defineProperty(o, prop, {
        value,
        configurable: true,
        writable: false,
        enumerable: true
      });
    }; // get read only data for this object
    super(address, abi, providerOrSigner);
    properties.defineReadOnly = defineReadOnly;
  }
...
exports.makeEthersBase = (artifact) => {
  ...
  const { defineReadOnly } = properties;
  properties.defineReadOnly = (o, prop, value) => {
    if (typeof value === 'function' && !prop.match(/\(/) && o.functions) generateAndAssignDefaultFunction(ethersBase, prop);
    else defineReadOnly.call(this, o, prop, value);
  }; // generate functions on the prototype
  new Contract(constants.AddressZero, artifact.abi, new BaseProvider());
  properties.defineReadOnly = defineReadOnly; // restore defineReadOnly
  return ethersBase;
};
